...
def baixar_cache_inicial():
    ...
    if not cache_path.exists():
        try:
            with st.spinner("🔄 Baixando dados iniciais do GitHub (primeira vez - 10MB)..."):
                # Baixar cache
                ...
                if r.status_code == 200:
                    ...
                    st.success("✅ Dados iniciais carregados com sucesso!")
                    return True
                else:
                    st.warning(f"⚠️ Cache não encontrado no GitHub (status {r.status_code})")
                    return False
        except Exception as e:
            st.warning(f"⚠️ Não foi possível baixar cache inicial: {e}")
            return False
    return True
...
# CARREGAR CACHE AUTOMATICAMENTE (UMA VEZ) - COM DOWNLOAD DO GITHUB
if 'dados_periodos' not in st.session_state:
    baixar_cache_inicial()  # Baixa do GitHub se não existir
...
if 'dados_periodos' in st.session_state and st.session_state['dados_periodos']:
    ...
    with col1:
        ...
        2. Use o menu lateral para acessar os dashboards
    ...
    with col2:
        ...
        **Depois:** Use o menu lateral ← para navegar
        
        **Atualizar:** Configure período e clique em "Extrair Novos Dados"
        
        **Explorar:** Acesse os dashboards disponíveis!
        """)
