...
def gerar_scorecard_pdf(banco_selecionado, df_banco, periodo_inicial, periodo_final):
    ...
    # Criar 2 linhas com 2 colunas cada
    metrics_table_data = [
        [
    ...
    # Função para criar mini gráfico matplotlib
    def criar_figura_grafico(df_plot, variavel, titulo):
        """Cria figura matplotlib para um gráfico"""
    ...
    for i in range(0, len(variaveis), figs_por_linha):
        ...
        for var in graficos_linha:
            try:
                ...
                # Converter matplotlib fig para Image reportlab
                img_buffer = io.BytesIO()
                fig.savefig(img_buffer, format='png', bbox_inches='tight', dpi=100)
                img_buffer.seek(0)

                # Criar objeto Image do reportlab a partir do BytesIO
                img = Image(img_buffer, width=2.2*inch, height=1.6*inch)
                row_images.append(img)

                plt.close(fig)
            except Exception as e:
                print(f"Erro ao gerar gráfico para {var}: {e}")
                row_images.append(Paragraph(f"[Erro ao gerar {var}]", styles['Normal']))
        ...
        with st.expander("🎨 editar cores das instituições"):
            ...
            for idx, instituicao in enumerate(instituicoes_scatter):
                with cols_cores[idx % 3]:
                    cor_padrao = obter_cor_banco(instituicao)
